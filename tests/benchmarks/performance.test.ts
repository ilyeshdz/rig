...
export async function benchmarkSmallSite(): Promise<BenchmarkResult> {
  ...
  try {
    ...
    const { duration } = await measurePerformance("build", () => 
...
export async function runAllBenchmarks(): Promise<void> {
  ...
  results.forEach(result => {
    ...
    console.log(`${result.operation.padEnd(35)} | ${result.duration.toFixed(2)}ms | ${(parseInt(files) / (result.duration / 1000)).toFixed(2)} files/sec`);
  });
  ...
  console.log("\n📈 Performance Analysis:");
  console.log(`Average time per file: ${((smallPerFile + mediumPerFile + largePerFile) / 3).toFixed(2)}ms`);
  console.log(`Scalability: ${largePerFile > smallPerFile * 2 ? '❌ Poor' : largePerFile > smallPerFile * 1.5 ? '⚠️  Needs improvement' : '✅ Good'}`);
}
